...
int main(int argc, char *argv[])
{
    ...
    mips_cpu_set_debug_level(c, 2, dest);
    ...
    mips_error err= mips_cpu_step(c);   // Execute the addu at address 4
    if (err)
        mips_cpu_set_pc(c, pc+4);
    ...
    // execute the jr at address 0
    if ((err=mips_cpu_step(c)))
        mips_cpu_set_pc(c, pc+4);

    if ((err=mips_cpu_step(c)))
        mips_cpu_set_pc(c, pc+4);

    // execute the addu in the delay slot at address 4
